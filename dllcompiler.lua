...
do
	...
	if success and type(source) == "string" and source ~= "" then
		local f, err = loadstring(source, "Advanced-Decompiler-V3")
		if f then
...
local function synsaveinstance(CustomOptions, CustomOptions2)
	...
	do
		...
		if OPTIONS.noscripts then
			ldecompile = function()
				return "-- Decompiling is disabled, turn it on and try again."
			end
		elseif Decompiler then
				...
				if ScriptCache then
					...
					if s then
						if not bytecode or bytecode == "" then
							return "-- The Script is Empty, yeah it's empty"
						end
					...
					if cached then
						if __DEBUG_MODE then
							warn("Found in Cache", script:GetFullName())
						end
						return cached
			...
			ldecompile = function()
				return "-- Decompiling is NOT supported on your executor, try turning decomtype to 'custom'"
			end
		end
	end
	...
	local function ReturnProperty(tag, propertyName, value)
		if type(value) ~= "string" and type(value) ~= "number" then
			...
			value = type(str) == "string" and str or ""
		end
	...
	local function save_cache()
		...
		if StatusText then
			StatusText.Text = "Saving Game.. File Size: " .. get_size_format()
		end
								...
								if PropertyName == "Source" then
									if DecompileIgnoring then -- ? Should this really prevent extraction of the original source if present ?
										...
										if should_decompile then
											...
											then
												value = "-- Server Scripts can NOT be saved, and will never be possible" --TODO: Could be not just server scripts in the future
											else
	...
	local function save_extra(name, hierarchy, customClassName, source)
		...
		local ok, err = xpcall(function()
			wrappedsaveinstance(ToSaveInstance, placename, CustomOptions)
		end, function(err)
			return err .. "\n" .. debug.traceback()
		end)
